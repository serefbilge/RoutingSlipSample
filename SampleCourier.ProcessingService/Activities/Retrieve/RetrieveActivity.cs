	...
	public class RetrieveActivity : Activity<RetrieveArguments,RetrieveLog>
	{
		...
		static string GetLocalFileName(HttpResponseMessage response,Uri sourceAddress)
		{
            ...
            if (response.Content.Headers.ContentDisposition != null)
            {
                ...
                {
                    localFileName += Path.GetExtension(fileName);
                }
            ...
            {
                throw new Exception("ContentDisposition is null!");
            }
		...
		{
			if (_logger.IsErrorEnabled) _logger.ErrorFormat("Hit here from fault in PostProcessActivity: {0}", context.Log.LocalFilePath);
